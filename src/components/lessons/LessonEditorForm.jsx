...
export default function LessonEditorForm({ lessonId = null }) {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (!apiKey) {
        toast.error(
          "ImgBB API key is missing. Please add NEXT_PUBLIC_IMGBB_API_KEY to .env",
        );
  ...
  return (
    <form
      ...
      className="w-full flex flex-col flex-grow gap-8"
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
        <div className="flex flex-col gap-2 relative">
          ...
          <select
            ...
            className="w-full h-11 bg-[#F6F0DD] border-[2.5px] border-[#1C1611] text-[#1C1611] text-xs font-black uppercase rounded-xl focus:outline-none px-4 appearance-none cursor-pointer shadow-[2px_2px_0px_0px_#1C1611]"
          >
          ...
          <ChevronDown className="w-4 h-4 text-[#1C1611] absolute right-4 bottom-3 pointer-events-none stroke-[2.5px]" />
        </div>
        ...
        <div className="flex flex-col gap-2 relative">
          ...
          <Smile className="w-4 h-4 text-[#1C1611] absolute right-4 bottom-3 pointer-events-none stroke-[2.5px]" />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-6 flex-grow min-h-[300px]">
            ...
            <label className="w-full h-32 border-[3px] border-dashed border-[#1C1611] rounded-xl flex flex-col items-center justify-center cursor-pointer bg-[#F6F0DD] hover:bg-white transition-colors group">
              ...
              {isUploadingImage ? (
                <div className="flex flex-col items-center gap-3">
                  <div className="w-8 h-8 border-[3px] border-[#1C1611] border-t-[#FF4A3A] rounded-full animate-spin" />
                  <span className="text-[10px] font-black uppercase text-[#1C1611]/70 tracking-wider">
                    Uploading...
                  </span>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-2 group-hover:-translate-y-1 transition-transform">
                  ...
                  <span className="text-xs font-black uppercase text-[#1C1611] tracking-wider">
                    Add Cover Image
                  </span>
                </div>
              )}
        ...
        <input
          ...
          className="w-full border-[3px] border-[#1C1611] p-4 text-xl font-black uppercase rounded-xl shadow-[4px_4px_0px_0px_#1C1611] bg-[#F6F0DD] text-[#1C1611] focus:outline-none"
        />

        <textarea
          value={content}
          onChange={(e) => setContent(e.target.value)}
          placeholder="Preserve your wisdom..."
          className="w-full flex-grow border-[3px] border-[#1C1611] p-4 text-sm font-bold resize-none min-h-[250px] rounded-xl shadow-[4px_4px_0px_0px_#1C1611] bg-[#F6F0DD] text-[#1C1611] focus:outline-none"
        />
      </div>

      <footer className="border-t-[3.5px] border-[#1C1611] pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            type="checkbox"
            checked={isPremium}
            onChange={(e) => setIsPremium(e.target.checked)}
            className="w-4 h-4 accent-[#FF4A3A]"
          />
